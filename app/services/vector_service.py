"""Qdrant vector store service for per-user document storage and retrieval"""
import asyncio
import logging
...
class QdrantVectorService:
    """
    Qdrant vector database service with per-user isolation
    """
    ...
    def __init__(self):
        ...
        self.default_collection = "user_documents"
        self.embedding_dimension = settings.embedding_dimension
    
    async def init_collection(self, collection_name: str = None) -> bool:
        """
        Initialize Qdrant collection with per-user support
        """
    ...
    async def add_documents(
        ...
        """
        Add documents to Qdrant with user isolation
        ...
        try:
            ...
            for doc in documents:
                ...
                # Ensure user_id is in payload for isolation
            ...
            logger.info(f"Added {len(points)} documents for user {user_id}")
            return True
            ...
            for result in search_results:
                results.append({
                    ...
                    "metadata": {
                        ...
                        if k not in ["user_id", "text", "document_id", "chunk_id", "source"]
                    }
                })
    ...
    async def delete_document(
        ...
        try:
            # Find all points for this document and user
            ...
            if scroll_result[0]:  # If points found
                ...
                logger.info(f"Deleted {len(point_ids)} chunks for document {document_id} for user {user_id}")
            
            return True
            
        except Exception as e:
            logger.error(f"Failed to delete document {document_id} for user {user_id}: {e}")
            return False
